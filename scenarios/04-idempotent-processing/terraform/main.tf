...
provider "aws" {
  ...
  default_tags {
    tags = { Project = "aws-serverless-typescript-playbook", Environment = var.environment, Scenario = "04-idempotent-processing", ManagedBy = "Terraform" }
  }
}

locals { name = "aws-playbook-idempotent-${var.environment}" }
...
module "worker" {
  ...
  artifact          = "${path.module}/../../../dist/04-idempotent-processing/handler.zip"
  environment       = { TABLE_NAME = aws_dynamodb_table.records.name }
  statements        = [{ actions = ["dynamodb:GetItem", "dynamodb:PutItem", "dynamodb:UpdateItem", "dynamodb:DeleteItem"], resources = [aws_dynamodb_table.records.arn] }, { actions = ["sqs:ReceiveMessage", "sqs:DeleteMessage", "sqs:GetQueueAttributes"], resources = [module.work.arn] }]
  timeout           = 15
}
