...
run "safe_defaults" {
  ...
  assert {
    condition     = !aws_lambda_event_source_mapping.worker.enabled && aws_lambda_event_source_mapping.worker.scaling_config[0].maximum_concurrency == 2
    error_message = "Workers must be disabled by default and bounded when enabled."
  }
  assert {
    condition     = aws_apigatewayv2_route.provider.authorization_type == "AWS_IAM"
