...
Nine executable serverless scenarios with strict TypeScript, AWS SDK v3, focused tests, sample events, and independent Terraform entry points. Each scenario can be studied and deployed on its own.
...
Run `pnpm check` for formatting, lint, strict TypeScript, unit tests, and Lambda bundles. `pnpm terraform:check` checks all nine infrastructure roots; `pnpm test:integration` exercises DynamoDB Local and HTTP timeouts. See [checks and coverage](docs/verification.md) for the command reference.
